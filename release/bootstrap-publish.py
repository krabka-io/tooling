#!/usr/bin/env python3
"""Publish every not-yet-published crate of a krabka-io workspace, in order.

The script reads the workspace, drops the crates crates.io already holds at
their current version, sorts the rest so a crate follows the crates it depends
on, and then asks before it publishes. It waits and retries when crates.io
rate-limits the account.
...
repository. Pass the workspace root as the first argument to name another one.
"""
import os
import re
import sys
...
def main():
    root_dir = os.path.abspath(sys.argv[1]) if len(sys.argv) > 1 else get_workspace_root()
